 ...
 * @see #dispose()
 */
public class FRIhandguide extends RoboticsAPIApplication
{	
    ...
    public void initialize()
    {
        ...
        mediaFlange.setOutputX3Pin2(false); // yellow
    }
    ...
    public void daniel(){
    	...
    	getLogger().info("Show modal dialog and wait for user to confirm");
        isCancel = getApplicationUI().displayModalDialog(ApplicationDialogType.QUESTION, "Grab the robot for handguiding. Stop app when done.", "OK", "Cancel");
        if (isCancel == 1)
        {
            return;
        }
