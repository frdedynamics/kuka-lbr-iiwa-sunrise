...
public class Motions_Window extends RoboticsAPIApplication {
	private LBR lbr;

	final static double radiusOfCircMove=120;
	final static int nullSpaceAngle = 80;
	
	final static double jointVelocityRel = 0.15;
	...
	double[] loopCenterPosition= new double[]{
			0, offsetAxis2And4 -Math.toRadians(10), 0, offsetAxis2And4 +offsetAxis4And6 -Math.toRadians(90), 0, offsetAxis4And6 -Math.toRadians(45),Math.toRadians(60)};
	...
	public void run() {		
        ...
        while(true) {
			...
			ptpToLoopCenter.setJointVelocityRel(jointVelocityRel);
			...
			Spline lemniscateSpline = createLemniscateSpline(startFrame).setJointJerkRel(0.5).setCartVelocity(125);
	
			getLogger().info("Execute lemniscate motion");
			lemniscateSpline.setJointVelocityRel(jointVelocityRel);
			...
			linToCenterFrameWithE1_1.setJointVelocityRel(jointVelocityRel);
			...
			linToCenterFrameWithE1_2.setJointVelocityRel(jointVelocityRel);
			lbr.move(linToCenterFrameWithE1_2);
			
			getLogger().info("Move to start position");
			LIN linToStartFrame = lin(startFrame);
			linToStartFrame.setJointVelocityRel(jointVelocityRel);
